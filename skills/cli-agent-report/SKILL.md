...
Save as `evaluations/<cli>/linkedin.md`. The post targets developers who build or run agents that call this CLI. It leads with the problem, immediately follows with the fix pattern, and ends with the report CTA.
...
---
If your agent uses "<cli>" today, <one-clause hook — e.g. "it's flying blind" / "it's silently failing" / "it's creating duplicates">.

I ran a live evaluation of "<cli>" against <N> Critical failure modes. <fail-count> score 0/3. <partial-count> score 1–2/3. Here's what's silently breaking your agent right now:

🔴 <Score-0 gap — one sentence describing what the agent experiences>

🔴 <Second score-0 or critical bug — agent's experience>

⚠️ <High-impact score-1 or score-2 gap — agent's experience>

Copy these into your agent's "<cli>" wrapper before shipping:

✅ <Plain-prose fix for first issue — one sentence, the approach not code>

✅ <Plain-prose fix for second issue — one sentence>

✅ <Plain-prose fix for third issue — one sentence>
...
- **Issues first, fixes second** — list all 🔴/⚠️ bullets together, then a "Here's how to fix each one:" section with matching ✅ bullets in the same order; never interleave problem and fix
- **Fix lines are prose patterns, never code** — describe the approach in one plain sentence ("query before mutate", "inspect stderr regardless of exit code"); code belongs in the integration guide, not the post
...
- **Scores near the top** — place pass/fail counts in the intro sentence after the hook, before the gap list
- **CTA points to integration guide and reports** — "full integration guide, runtime brief, and issues report"
...
---
**1/**
If your agent uses `<cli>` — <one-clause hook, max 180 chars to leave room for 🧵>.

I ran a live evaluation: <N> Critical failure modes against the real binary. Here's what's broken.

🧵
---

**2/**
🔴 <Score-0 issue — agent's experience, one sentence. ≤ 280 chars.>
---

**3/**
🔴 <Second score-0 or confirmed bug — agent's experience. ≤ 280 chars.>
---
...
✅ <Plain-prose fix for post 2 — one line>
✅ <Plain-prose fix for post 3 — one line>
✅ <Plain-prose fix for post 4 — one line>
---

**6/ (closing)**
<fail> Critical gaps at zero. <partial> partial. Full integration guide + runtime brief →
...
- **Issues first, fixes second** — issue posts (2–4) list only the problem; a dedicated fixes post (5) lists all ✅ fixes in matching order; never interleave
- **Fix lines are prose patterns, never code** — one sentence describing the approach; code belongs in the integration guide
