...
for schema_md in "$ROOT"/schemas/*.md; do
  ...
  while IFS= read -r req_id; do
    ...
    grep -q "${schema_base}.md" "$req_file" || {
      echo "MISSING BACK-LINK: $req_id has no link to $schema_base.md in its ## Schema section  (schema: $base)"
      ERRORS=$((ERRORS + 1))
    }
...
# AGENTS.md
check_count "$ROOT/AGENTS.md"  "[0-9]+ failure modes, [0-9]+ requirements" \
...
- `INCOMPLETE` → add the missing section(s) to the file; see AGENTS.md for required section order
