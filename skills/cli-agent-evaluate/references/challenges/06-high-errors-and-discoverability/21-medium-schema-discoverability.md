...
| 1 | Bare invocation exits 0 with root help; `--help --format json` returns some structured info but no output schema; commands not enumerable from root |
| 2 | Bare invocation exits 0; `tool --schema --format json` returns command list with parameter types, required flags, and exit codes |
...
**Check:** First run `tool` with no arguments and confirm exit 0. Then run `tool --schema --format json` and verify the response includes `commands[].parameters[].type` and `commands[].output_schema` for at least one command.
