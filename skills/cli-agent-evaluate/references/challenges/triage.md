...
| 2 | `exit 0` but output text indicates failure, or a pipeline swallowed the code | [§1](04-critical-output-and-parsing/01-critical-exit-codes.md) [§56](01-critical-ecosystem-runtime-agent-specific/56-high-pipeline-exit-masking.md) | Classify by `meta.exit_code` when the envelope has one; on any disagreement, failure wins; check `PIPESTATUS`; add `set -o pipefail` |
