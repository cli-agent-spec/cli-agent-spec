...
One line of the JSONL stream consumed by the built-in `exec` command. It routes to a subcommand by `_cmd`, applies per-line flag overrides from `_opts`, and forwards remaining fields as the `--input` JSON payload. Batch dispatch replaces N process spawns with one (§77); the underscore prefix keeps routing keys from colliding with payload fields.

Each answering output line is a `ResponseEnvelope` carrying `meta._cmd` and `meta._line`, so failures map back to input lines without a parallel counter.
...
```json
{"_cmd": "transaction.add", "_opts": {"draft": true, "target": "inbox.bc"}, "date": "2024-01-15", "narration": "Buy BTC", "postings": [{"account": "Assets:Bank", "amount": "-100 USD"}]}
```
