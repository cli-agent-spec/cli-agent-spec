...
- §69 (Argument Order Ambiguity): MCP ✓ — JSON-based invocation has no argument ordering concerns by design. All parser-based frameworks are ~ — mechanisms exist (Cobra persistent flags, argparse parent parsers) but global options after the command path are not on by default.
